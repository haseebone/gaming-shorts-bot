...
## Engagement upgrades (for more views)

- **Punchy hooks**: `generate_script.py` opens each script with a varied
  attention-grabbing line ("Big gaming news —", "The results are in —",
  etc.) instead of flatly restating the headline, and keeps the body
  tight (3 sentences) for a snappier pace. Hooks rotate randomly so
  consecutive videos don't all sound the same.
- **Bold, boxed captions**: large, bold, high-contrast captions with a
  semi-transparent background box (the CapCut/TikTok look) instead of
  small plain subtitle text — much easier to read at a glance while
  scrolling.
- **Background music**: a free, public-domain (CC0, zero attribution
  required) instrumental track plays quietly under the narration,
  downloaded automatically each run from the Internet Archive's mirror
  of the freepd.com collection. If a track fails to download for any
  reason, the video still renders fine without music rather than
  failing the run.
